...
# Module-level configuration (will be set by configure_globals before model init)
IMAGE_SIZE = 1024
BASE_SIZE = 1280
CROP_MODE = True
...
class DeepseekOCRForCausalLM(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    @classmethod
    def configure_globals(cls, image_size: int = 1024, base_size: int = 1280,
                         crop_mode: bool = True, print_tokens: bool = False):
        """
        Configure module-level settings before model initialization.

        This method updates the global configuration variables that are used
        by vLLM's internal processing pipeline.

        Args:
            image_size: Crop tile size for Gundam mode (default: 1024)
            base_size: Global view size for Gundam mode (default: 1280)
            crop_mode: Enable dynamic preprocessing (default: True)
            print_tokens: Debug flag for printing token counts (default: False)
        """
        import sys
        current_module = sys.modules[__name__]
        current_module.IMAGE_SIZE = image_size
        current_module.BASE_SIZE = base_size
        current_module.CROP_MODE = crop_mode
        current_module.PRINT_NUM_VIS_TOKENS = print_tokens
